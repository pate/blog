...
namespace Blog.Web.Helpers
{
    ...
    {
        public const string Descriptive = @"d MMM \'yy a\t HH:mm";
    }
	...
	public class FuzzyDateTime
	{
		...
		static readonly List<IDateTimePattern> parsers = new List<IDateTimePattern>()
		{
		   ...
		   new RegexDateTimePattern (
				@"(last|next) *({0}).*".Fmt(String.Join("|", dayList.ToArray())), //handle weekdays
				delegate (Match m) {
		   ...
		   new RegexDateTimePattern (
				...
				delegate (Match m) {
					DateTime dt;
					int direction = (m.Groups[1].Value == "last")? -1 :1;
					var s = "{0} {1}".Fmt(m.Groups[2].Value, DateTime.Now.Year + direction);
	...
	public class RegexDateTimePattern : IDateTimePattern
	{
